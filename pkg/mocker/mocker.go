...
type mocker struct {
	src, dst, prefix, suffix, pkg string
	iface                         []string
}

func New(src, dst, pkg, prefix, suffix string, iface []string) (*mocker, error) {
	return &mocker{
		src:    src,
		dst:    dst,
		pkg:    pkg,
...
func (m *mocker) Mock() error {
	...
	g.prefix = m.prefix
	g.suffix = m.suffix
	if err := g.Generate(pkg, m.pkg, outputPackagePath); err != nil {
		return err
	}
...
func (g *generator) Generate(pkg *model.Package, pkgName, outputPkgPath string) error {
