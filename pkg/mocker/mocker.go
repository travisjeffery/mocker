...
func New(src, dst, pkg, prefix, suffix, selfpkg string, interfaces []string) (*mocker, error) {
	return &mocker{
		src:        src,
		dst:        dst,
		mockPkg:    pkg,
...
func (m *mocker) Mock() error {
	...
	g := &generator{}
	g.src = m.src
	g.prefix = m.mockPrefix
	g.suffix = m.mockSuffix
	if err := g.Generate(me.pkg, m.interfaces, outputPackagePath); err != nil {
		return err
	}
...
type generator struct {
	buf            bytes.Buffer
	pkgMap         map[string]string // import path to mockPkg name
...
func (g *generator) Output() []byte {
	...
	src, err := format.Process(g.src, g.buf.Bytes(), options)
	if err != nil {
...
func (g *generator) Generate() error {
	...
	node, err := parser.ParseFile(fset, g.src, nil, 0)
	if err != nil {
		return err
	}

	// pkg name to import path
	iimports := make(map[string]string)
	iinterfaces := make(map[string]*ast.InterfaceType)

	ast.Inspect(node, func(n ast.Node) bool {
		switch t := n.(type) {
		case *ast.ImportSpec:
			if t.Name.IsExported() {
				iimports[t.Name.String()] = t.Path.Value
			}
		case *ast.TypeSpec:
			if t.Name.IsExported() {
				switch tt := t.Type.(type) {
				case *ast.InterfaceType:
					_, ok := g.interfaces[t.Name.String()]
					if ok || len(g.interfaces) == 0 {
						iinterfaces[t.Name.String()] = tt
					}
	...
	// interfaces
	for _, iinterface := range iinterfaces {
		if err := g.GenerateInterface(iinterface); err != nil {
			return err
		}
	}
...
func (g *generator) GenerateInterface(iinterface *ast.InterfaceType) error {
	mockType := g.mockName(iinterface.Name)

	g.p("")
	g.p("// %v is a mock of %v interface", mockType, iinterface.Name)
	g.p("type %v struct {", mockType)
	g.in()

	for _, m := range iinterface.Methods {
		g.p("lock%v sync.Mutex", m.Name)

		argNames := g.getArgNames(m)
		argTypes := g.getArgTypes(m, g.selfPkg)
		argString := makeArgString(argNames, argTypes)

		rets := make([]string, len(m.Out))
		for i, p := range m.Out {
			rets[i] = p.Type.String(g.pkgMap, g.selfPkg)
		}
	...
	return g.GenerateMethods(mockType, intf, g.selfPkg)
}
...
// 		return mockName
// 	}
// 	return g.mockPrefix + typeName + g.mockSuffix
// }
